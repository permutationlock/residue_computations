...
for Vpair in ResidueList do
    ...
    if OK cmpeq IntegerRing() then
      S:=PrimeFactors(&*[Numerator(c)*Denominator(c) where c:=LeadingCoefficient(Numerator(a)): a in V]);
    else
      S:=Support(&*[Numerator(c)*Denominator(c) where c:=LeadingCoefficient(Numerator(a)): a in V]*OK);
    end if;
    Append(~W,[[Valuation(v,p): p in FnFactorBasis]cat [Valuation(LeadingCoefficient(Numerator(v)),p):p in S] cat [Sign(Rationals()!Numerator(c)*Denominator(c)) where c:=LeadingCoefficient(Numerator(v))]:v in V]);
...
```{magma}
[[-1,2,3,T, T+1, T-1, T+3, T-3, -1,2,3,T, T+1, T-1, T+3, T-3][i]: i in [1..#Eltseq(b)] | b[i] eq 1] where b:=Basis(bs)[3];
```
